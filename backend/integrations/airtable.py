...
_missing = [
    name for name, val in [
        ...
        ('AIRTABLE_REDIRECT_URI', REDIRECT_URI),
    ]
    if not val
]
if _missing:
...
scope = os.environ.get('AIRTABLE_SCOPES', 'data.records:read schema.bases:read')
        ...
        response, _, _ = await asyncio.gather(
            client.post(
                ...
                data={
                    ...
                    'client_id': CLIENT_ID,
                    'code_verifier': code_verifier.decode('utf-8') if isinstance(code_verifier, (bytes, bytearray)) else code_verifier,
                },
                headers={
...
async def fetch_items(
    access_token: str, url: str, aggregated_response: list, offset=None, client=None
) -> dict:
    ...
    if response.status_code == 200:
        ...
        if offset is not None:
            await fetch_items(access_token, url, aggregated_response, offset, client)
        else:
            return
...
async def get_items_airtable(credentials) -> list[IntegrationItem]:
    ...
    async with httpx.AsyncClient() as client:
        await fetch_items(credentials.get('access_token'), url, list_of_responses, client=client)
